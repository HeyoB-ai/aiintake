...
const FORBIDDEN: readonly { pattern: RegExp; why: string }[] = [
  {
    ...
    why: 'de service-role key omzeilt RLS volledig en hoort niet in een langlevend proces',
  },
  {
    pattern: /\bservice_role\b/,
    why: 'verwijzing naar de service_role',
  },
  {
  ...
  },
  {
    ...
    why: 'readServerEnv valideert onder meer de service-role key en het JWT-secret',
  },
  {
    pattern: /SUPABASE_JWT_SECRET/,
    why: 'met het JWT-secret kan de worker zelf tokens minten voor willekeurige intakes',
  },
  {
    pattern: /mintAgentToken/,
    why: 'tokens minten is de taak van de web-app; de worker ontvangt er één',
  },
];
...
describe('apps/agent heeft geen service-role key', () => {
  ...
  it('de env-sjabloon van de agent noemt de service-role key niet', () => {
